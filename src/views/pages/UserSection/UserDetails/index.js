...
class UserDetails extends Component {
  ...
  getSigleUserData = async () => {
    ...
    try {
      const response = await getSignleUserInfo(this.props.route.params.userData.id);
      ...
      if (response.isSuccess) {
        ...
        this.setState({
          ...
          selectedProfileImage: data.is_profile_pic ? data.is_profile_pic : data.usr_profile_photo[0].picture_url,
          profileImageArray: data.usr_profile_photo.length ? response.result.success.usr_profile_photo : [],
        });
        if (response.result.success.tweets && response.result.success.tweets.length > 0) {
    ...
    return this.state.userReview.length ?
      this.state.userReview.map((item, index) => (
        <View
          ...
          style={{ borderTopColor: '#ddd', borderTopWidth: 1, flexDirection: 'row', padding: 15, justifyContent: 'space-between' }}
        >
          <View>
            ...
            <SafeAreaView style={styles.Ccontainer}>
              <ScrollView style={styles.scrollView}>
                <View style={styles.bgColor}>
                  <ImageBackground
                    source={this.state.selectedProfileImage ? { uri: this.state.selectedProfileImage } : require('../../../../assets/profile/blackG.png')}
                    style={styles.image}
                  >
                      ...
                      <View style={styles.profileTextContainer}>
                        <View style={{ flexDirection: 'row' }}>
                          ...
                          <Text numberOfLines={1} style={styles.textOnImage}>
                            {this.state.userData ? this.state.userData.usr_nickname : null}
                          </Text>
                    ...
                    <View style={styles.paddingTop20}>
                      {this.state.userData && this.state.userData.basic_info.height &&
                        <View style={styles.basInfo}>
                          ...
                          <Text style={styles.basInfoValue}>
                            {this.state.userData ? this.state.userData.basic_info.height : null}
                          </Text>
                        </View>
                      }
                      {this.state.userData && this.state.userData.basic_info.location &&
                        <View style={styles.basInfo}>
                          <Text>居住地</Text>
                          <Text style={styles.basInfoValue}>
                            {this.state.userData ? this.state.userData.basic_info.location : null}
                          </Text>
                        </View>
                      }
                      {this.state.userData && this.state.userData.basic_info.sake_drink &&
                        <View style={styles.basInfo}>
                          <Text>タバコ</Text>
                          <Text style={styles.basInfoValue}>
                            {this.state.userData ? this.state.userData.basic_info.sake_drink : null}
                          </Text>
                        </View>
                      }
                      {this.state.userData && this.state.userData.basic_info.smoking &&
                        <View style={styles.basInfo}>
                          <Text>同居人</Text>
                          <Text style={styles.basInfoValue}>
                            {this.state.userData ? this.state.userData.basic_info.smoking : null}
                          </Text>
                        </View>
                      }
                      {this.state.userData && this.state.userData.basic_info.hair_color &&
                        <View style={styles.basInfo}>
                          <Text>髪の色</Text>
                          <Text style={styles.basInfoValue}>
                            {this.state.userData ? this.state.userData.basic_info.hair_color : null}
                          </Text>
                        </View>
                      }
                      {this.state.userData && this.state.userData.basic_info.hair_style &&
                        <View style={styles.basInfo}>
                          <Text>髪型</Text>
                          <Text style={styles.basInfoValue}>
                            {this.state.userData ? this.state.userData.basic_info.hair_style : null}
                          </Text>
                        </View>
                      }
                      {this.state.userData && this.state.userData.basic_info.work &&
                        <View style={styles.basInfo}>
                          <Text>職業</Text>
                          <Text style={styles.basInfoValue}>
                            {this.state.userData ? this.state.userData.basic_info.work : null}
                          </Text>
                        </View>
                      }
                      {this.state.userData && this.state.userData.basic_info.annual_income &&
                        <View style={styles.basInfo}>
                          <Text>年収</Text>
                          <Text style={styles.basInfoValue}>
                            {this.state.userData ? this.state.userData.basic_info.annual_income : null}
                          </Text>
                        </View>
                      }
                      {this.state.userData && this.state.userData.basic_info.educational_background &&
                        <View style={styles.basInfo}>
                          <Text>学歴</Text>
                          <Text style={styles.basInfoValue}>
                            {this.state.userData ? this.state.userData.basic_info.educational_background : null}
                          </Text>
                        </View>
                      }
                      {this.state.userData && this.state.userData.basic_info.place_of_birth &&
                        <View style={styles.basInfo}>
                          <Text>出生地</Text>
                          <Text style={styles.basInfoValue}>
                            {this.state.userData ? this.state.userData.basic_info.place_of_birth : null}
                          </Text>
                        </View>
                      }
                      {this.state.userData && this.state.userData.basic_info.annual_income &&
            ...
            <View style={styles.content2}>
              ...
              <View style={styles.optionHolder}>
                <TouchableWithoutFeedback onPress={() => this.GiveReview()}>
                  <View style={{ ...styles.options, backgroundColor: '#6678AB', color: 'black' }}>
                    <Text style={{ textAlign: 'center', color: 'white', fontSize: 15, fontWeight: 'bold' }}>
                      レビュー送信
                  </Text>
                  </View>
                </TouchableWithoutFeedback>
                <TouchableWithoutFeedback onPress={() => this.setState({ userReviewModal: false })}>
